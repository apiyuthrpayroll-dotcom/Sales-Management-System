...
import { Download, Plus, Search, FileText, CheckCircle2, XCircle, Trash2, Edit, Printer, Copy, RefreshCw, LayoutDashboard, List, Send, Filter, ChevronUp, ChevronDown, TrendingUp } from 'lucide-react';
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30 px-6 py-4 print:hidden">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
             <button onClick={() => { setEditingId('new'); }} className="px-4 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-bold shadow-sm hover:bg-blue-700 flex items-center gap-2 cursor-pointer">
                <Plus className="w-4 h-4"/> Create Quotation
             </button>
             <button onClick={() => window.location.href='/'} className="px-4 py-2.5 bg-white border border-slate-300 text-slate-700 rounded-lg text-sm font-bold shadow-sm hover:bg-slate-50 cursor-pointer">
                Back to ERP
             </button>
          </div>
        </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <KPICard title="Total Quotations" value={dashboardData.total} subtitle="All time" icon={<FileText className="w-5 h-5 text-blue-600"/>} bg="bg-blue-50/70" border="border-blue-100" />
                <KPICard title="Total Value" value={`฿${dashboardData.value.toLocaleString(undefined, {minimumFractionDigits: 2})}`} subtitle="Grand Total Amount" icon={<TrendingUp className="w-5 h-5 text-emerald-600"/>} bg="bg-emerald-50/70" border="border-emerald-100" />
                <KPICard title="Approved & Won" value={dashboardData.approved} subtitle="Won deals" icon={<CheckCircle2 className="w-5 h-5 text-indigo-600"/>} bg="bg-indigo-50/70" border="border-indigo-100" />
                <KPICard title="Pending Review" value={dashboardData.pending} subtitle="Awaiting decision" icon={<RefreshCw className="w-5 h-5 text-amber-600"/>} bg="bg-amber-50/70" border="border-amber-100" />
              </div>
              
              {/* Status Pie Chart - Right Column */}
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">Status Distribution</h3>
                ...
                <div className="h-[140px] flex items-center justify-center">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      ...
                      <RechartsTooltip contentStyle={{borderRadius: '8px', border: 'none', fontSize: '11px', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)'}} />
                    </PieChart>
                  </ResponsiveContainer>
                </div>
                {/* Custom Legend */}
...
function QuoteList({ quotations, onEdit, onPrint, onDuplicate, onDelete, onRefresh }: any) {
               ...
               <tr key={q.id} className="hover:bg-slate-50/80 transition-colors">
                 ...
                 <td className="py-3 px-4 text-center">
                    <div className="flex items-center justify-center gap-2">
                       ...
                       <button onClick={() => onDelete(q.id)} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded cursor-pointer" title="ลบใบเสนอราคา (Delete)"><Trash2 className="w-4 h-4"/></button>
